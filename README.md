...
- `opt` - a structure array. It is the extra options for `printInp2d`. It stores extra parameter settings for `printInp2d`. `opt.user_nodeSet` is used to defined customized boundary node set (see comments in `printInp2d.m`).

**Notes:**

- Function `printInp2d` will automatically export boundary node sets.
